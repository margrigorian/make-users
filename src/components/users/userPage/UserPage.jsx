...
function UserPage(props) {
    ...
    async function getPosts(id, status) {
        let posts = await request("GET", `https://jsonplaceholder.typicode.com/posts?userId=${id}`);
        setData(posts);
        setStatus(status);
    }

    async function getAlbums(id, status) {
        let albums = await request("GET", `https://jsonplaceholder.typicode.com/albums?userId=${id}`);
        setData(albums);
        setStatus(status);
    }
        ...
        <ModeContext.Consumer>
            {
                (mode) => (
                        <div className={s.body}>
                            {status === "" ? 
                                <div style={{backgroundColor: mode === "night" ? "#0a0a0a" : undefined}}>
                                    <button className={s.buttonBack} onClick={() => props.fnComeBack()} style={{backgroundColor: mode === "night" ? "#470404" : undefined, color: mode === "night" ? "white" : undefined}}>&#8666; BACK</button>
                                    <div className={s.userContainer} style={{backgroundColor: mode === "night" ? "#cccaca" : undefined}}>
                                        <div>
                                            <img src={mode === "night" ? "https://i.gifer.com/Cag6.gif" : "http://26.media.tumblr.com/tumblr_ltusgeaJ8H1qf9jcfo1_500.gif"} alt="User" className={s.userPhoto}/>
                                            <p className={s.userName} style={{color: mode === "night" ? "darkRed" : undefined}}>{props.userInfo.name}</p>
                                            <p style={{color: mode === "night" ? "darkRed" : undefined}}><span className={s.colorText} style={{color: mode === "night" ? "#1a1818" : undefined}}>User Name: </span>{props.userInfo.username}</p>
                                            <p style={{color: mode === "night" ? "darkRed" : undefined}}><span className={s.colorText} style={{color: mode === "night" ? "#1a1818" : undefined}}>Email: </span>{props.userInfo.email}</p>
                                            <p style={{color: mode === "night" ? "darkRed" : undefined}}><span className={s.colorText} style={{color: mode === "night" ? "#1a1818" : undefined}}>Phone: </span>{props.userInfo.phone}</p>
                                            <p style={{color: mode === "night" ? "darkRed" : undefined}}><span className={s.colorText} style={{color: mode === "night" ? "#1a1818" : undefined}}>Address: </span>{props.userInfo.address.street}</p>
                                        </div>
                                        <div>
                                            <button className={s.button} style={{backgroundColor: mode === "night" ? "#470404" : undefined, border: mode === "night" ? "1px solid darkRed" : undefined}} onClick={() => getPosts(props.userInfo.id, "posts")}>POST</button>
                                            <button className={s.button} onClick={() => getAlbums(props.userInfo.id, "albums")}>ALBUM</button>
                                        </div>
                                    </div>
                                </div> : 
                                    (status === "posts" ? <PostsList posts={data} fnBackToUsers={props.fnComeBack} fnBackToUserPage={backToUserPage} /> 
                                        : <AlbumsList albums={data} fnBackToUsers={props.fnComeBack} fnbackToUserPage={backToUserPage}/>)
                            }
                        </div>
                )
            }
